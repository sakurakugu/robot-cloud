<template>
  <div class="keyframe-track" @click="onTrackClick">
    ...
    <!-- 空状态 -->
    <div v-if="keyframes.length === 0" class="empty-track">点击添加关键帧</div>
  </div>
</template>

<script setup lang="ts">
import { computed } from 'vue';
import type { Keyframe, TimelineConfig, Track } from '../../types';
...
const onDragKeyframe = (e: MouseEvent) => {
  ...
  // 更新关键帧
  const updatedKeyframes = keyframes.value.map((kf) =>
    kf.id === draggedKeyframe!.id ? { ...kf, time: newTime, value: newValue } : kf
  )
