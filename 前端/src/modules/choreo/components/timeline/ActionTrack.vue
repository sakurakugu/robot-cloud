<template>
  <div class="action-track" @click="handleTrackClick">
    ...
    >
      ...
      <!-- 左侧调整手柄 -->
      <div class="resize-handle left" @mousedown.stop="startResize($event, block, 'left')"></div>
      <!-- 右侧调整手柄 -->
      <div class="resize-handle right" @mousedown.stop="startResize($event, block, 'right')"></div>
    </div>

    <!-- 空状态提示 -->
    <div v-if="blocks.length === 0" class="empty-hint">点击轨道空白处添加动作块</div>
  </div>
</template>
