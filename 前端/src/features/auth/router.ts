...
export const authLayoutRoutes: RouteRecordRaw[] = [
  {
    path: '/personal',
    name: 'PersonalCenter',
    component: () => import('@/modules/auth/views/PersonalCenter.vue'),
    meta: { title: '个人资料', requireAuth: true, icon: 'User' },
  },
  {
    path: '/sessions',
    name: 'LoginDevices',
    component: () => import('@/modules/auth/views/LoginDevices.vue'),
    meta: { title: '登录设备', requireAuth: true, icon: 'Monitor' },
  },
  {
    path: '/users',
    name: 'UserManage',
    component: () => import('@/modules/auth/views/UserManage.vue'),
